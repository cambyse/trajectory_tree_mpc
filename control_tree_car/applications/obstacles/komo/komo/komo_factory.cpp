...
Objectives KomoFactory::ground_komo(const std::shared_ptr< KOMO > & komo, const std::vector<Obstacle> & obstacles, double road_width, double v_desired) const
{
    ...
    objectives.ax_ = komo->addObjective(-123., 123., new AxisBound("car_ego", AxisBound::Y, AxisBound::EQUAL, komo->world), OT_sos, NoArr, 5.0 * 1e-1, 0); // STRONG INFLUENCE ON RUNTIME!
