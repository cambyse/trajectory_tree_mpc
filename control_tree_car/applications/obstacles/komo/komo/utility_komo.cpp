...
void shift_komos(std::vector<std::shared_ptr<KOMO>>& komos, const OdometryState & o, uint steps)
{
    ...
    if(index >= 2) // is komo order
    {
        ...
        for(auto&komo: komos)
        {
            ...
            if(vx_end > 1.0)
            {
               slide_trajectory(index - 2, mu, steps, traj);
            ...
            {   // unstuck trajectory
                ...
                first_guess_trajectory(o, 2.0, steps, traj);
            }
...
//        first_guess_trajectory(o, 1.0, steps, traj);
//    }
...
{
    //std::cout << "slide_trajectory.." << shift << std::endl;
...
void slide_trajectory(uint shift, double mu, uint steps, std::vector<Pose2D>& trajectory)
{
    //std::cout << "slide_trajectory smooth.." << shift << " " << mu << std::endl;
    ...
    for(auto i = 0; i < trajectory.size() - shift - 1; ++i)
    {
        trajectory[i] = interpolate(trajectory[i+shift], trajectory[i+shift+1], mu);
    }

    for(auto i = 0; i <= shift; ++i)
    {
        const auto I = trajectory.size() - shift + i - 1;
        trajectory[I].x = last.x + (i + mu) * dx;
        trajectory[I].y = last.y + (i + mu) * dy;
        trajectory[I].yaw = yaw;
    }
}
...
Pose2D interpolate(const Pose2D& a, const Pose2D& b, double mu)
{
  ...
  ret.x = a.x * (1 - mu) + mu * b.x;
  ret.y = a.y * (1 - mu) + mu * b.y;
  ret.yaw = a.yaw * (1 - mu) + mu * b.yaw;
...
{
    //std::cout << "translate_trajectory.." << std::endl;
